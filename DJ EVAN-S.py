...
rgen=f' {K}[{RED}√{K}] {P}'
wt=f' {K}[{GREEN}?{K}] {P}'
fst=f'{dot}[{H}sathi{M}={H}abir{M}={H}tisha{M}={H}evan{M}={H}samim{P}]'
...
def banner():
 ...
 |_____/ \____/  |______|   \/_/    \_\_| \_|
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{warna} EVAN-VAW                 {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}EVAN- Chowdhury              {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+8801959440200         {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/cyberrafi-31{N}    {H}  ║
...
def mainx():
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/FB-KING");Evan()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/profile.php?id=100093687466670
     ...
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/FB.KING.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");evan()
def mainx2():
    os.system("clear")
    banner()
...
if __name__=='__EVAN__':
  try:os.mkdir('KING-EVAN')
  except:pass
  try:os.mkdir('QUEEN-JANNAT')
  except:pass
#  b2()
#  Evanx() 
  Evanx()